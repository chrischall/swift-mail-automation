...
- **Search backend selection.** `search` picks the backend automatically: Spotlight when `spotlight != nil` and no account/mailbox scope or non-zero `offset` was supplied (Spotlight can't scope by Mail's account/mailbox grouping and can't page); AppleScript otherwise. `forceBackend: .spotlight` skips the AppleScript fallback even on empty results. Don't collapse this — both constraints matter.
